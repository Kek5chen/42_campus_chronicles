...
void Sprite2D::set_texture(SDL_Surface* surface)
{
	...
	this->_size.x = this->_surface->w / 4;
	this->_size.y = this->_surface->h / 4;
...
void Sprite2D::draw()
{
	...
	if (!this->_game)
		return;
	rect.x = this->pos.x - this->_game->camera.pos.x - this->_size.x / 2 + this->_game->get_width() / 2;
	rect.y = this->pos.y - this->_game->camera.pos.y - this->_size.y / 2 + this->_game->get_height() / 2;
	rect.w = this->_size.x;
	rect.h = this->_size.y;
